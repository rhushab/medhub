...
public class ChatController {
    ...
    public ResponseEntity<ResponseModel<List<ChatDetail>>> getConversation(@RequestParam("receiverUserId") String receiverUserId, Authentication authentication) {
        String userName=authentication.getName();
        return ResponseEntity.ok(this.chatService.getConversationBetweenTwoUsers(userName, receiverUserId));
    }
    ...
    public ResponseEntity<ResponseModel<List<UserDetail>>> getChatPartners(Authentication authentication) {
        String userName=authentication.getName();
        return ResponseEntity.ok(this.chatService.findAllChatPartnersByUserId(userName));
    }
