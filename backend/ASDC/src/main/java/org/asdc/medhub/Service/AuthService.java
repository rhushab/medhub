...
public class AuthService implements IAuthService {
    ...
    public AuthService(IJwtService jwtService,
                       ...
                       EmailService emailService, CustomConfigurations customConfigurations
    ) {
    ...
    public ResponseModel<String> registerUser(DoctorRegistrationModel doctorForRegistration){
        ...
        if(!isDoctorExists){
            ...
            String verificationLink = this.customConfigurations.frontEndAppUrl+
                    "/auth/api/verify" + "?email=" + user.getUsername() +
    ...
    public ResponseModel<String> registerUser(PatientRegistrationModal patientForRegistration){
        ...
        var patientEmail= patientForRegistration.getUsername();
        var userRole=patientForRegistration.getUserRole();

        //Check if patient already exists or not
        var isDoctorExists=this
                .userRepository
                .existsUserByUsernameAndUserRole(patientEmail,userRole);
    ...
    public ResponseModel<String> registerUser(PharmacistRegistrationModel pharmacistRegistrationModel){
        ResponseModel<String> response=new ResponseModel<String>();
        var pharmacistEmail= pharmacistRegistrationModel.getUsername();
        var userRole=pharmacistRegistrationModel.getUserRole();
        //Check if patient already exists or not
        var isPharmacistExist=this
                .userRepository
                .existsUserByUsernameAndUserRole(pharmacistEmail,userRole);
    ...
    public ResponseModel<String> signInUser(User user){
        ...
        user.hashPassword();
        boolean isAuthenticationSuccesful=this
                .userRepository
        ...
        if(isAuthenticationSuccesful){
            User dbUser = userRepository.findUserByUsername(user.getUsername());
            if (this.isUserDoctorOrPharmacist(dbUser) && this.isVerificationPending(dbUser)) {
                response.message = "User is yet to be admin approved.";
            } else if (this.isUserDoctorOrPharmacist(dbUser) && this.isVerificationRejected(dbUser)) {
                response.message = "You are rejected by MedHub admin. Please follow steps on mail for further process";
            } else {
                String userName=dbUser.getUsername();
                String userId=Long.toString(dbUser.getId());
                UserRole userRole=dbUser.getUserRole();

                response.responseData = this.jwtService.generateToken(userName,userId,userRole);
    ...
    /**
     * Checks if user is pharmacist or doctor
     * @param user User object to check
     * @return boolean if yes
     */
    private boolean isUserDoctorOrPharmacist(User user){
        return UserRole.DOCTOR.equals(user.getUserRole()) || UserRole.PHARMACIST.equals(user.getUserRole());
    }

    /**
     * Checks if admin verification for user pending or not
     * @param user User to check
     * @return Boolean status
     */
    private boolean isVerificationPending(User user){
        return user.getAdminVerificationStatus().equals(AdminVerificationStatus.PENDING);
    }

    /**
     * Checks if admin verification for user is rejected or not
     * @param user User to check
     * @return boolean status
     */
    private boolean isVerificationRejected(User user){
        return user.getAdminVerificationStatus().equals(AdminVerificationStatus.REJECTED);
    }
    ...
    public ResponseModel<String> resetPassword(String email, String resetToken, String newPassword) {
        ...
        if (this.isResetPassParamsValid(email,resetToken,newPassword)) {
            ...
            response.responseData="Invalid input fields. Please check it.";
        }
        return response;
    }

    /**
     * Check if parameters values are valid or not
     * @param email Email
     * @param resetToken reset token
     * @param newPassword new password string
     * @return boolean
     */
    private boolean isResetPassParamsValid(String email,String resetToken,String newPassword){
        return !email.isEmpty() && !resetToken.isEmpty() && !newPassword.isEmpty();
    }
    ...
    private void sendResetEmail(String email, String resetToken) {
        ...
        String resetLink = this.customConfigurations.frontEndAppUrl + "/reset-password?token=" + resetToken + "&email=" + email;
