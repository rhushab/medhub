...
public class PatientService implements IPatientService {
     ...
     */
    private final MedicalSpecializationRepository medicalSpecRepo;
    ...
    public PatientService(UserRepository userRepository,
                          ...
                          MedicalSpecializationRepository medicalSpecRepo,
                          AppointmentRepository appointmentRepository,
                          ICommonService commonService) {
        this.userRepository = userRepository;
        this.patientRepository = patientRepository;
        this.doctorRepository=doctorRepository;
        this.medicalSpecRepo = medicalSpecRepo;
    ...
    public ResponseModel<List<String>> getMedicalSpecializationByName(String searchString){
        ...
        try {
            var medicalSpecializationList=this.medicalSpecRepo
    ...
    private HashMap<String,List<String>> getWeekWiseAvailabilityOfDoctor(Doctor doctor){
        ...
        long startTime= this.roundTo30Minutes((
                (doctor.getStartTime() != null)
                    ? doctor.getStartTime(): ProjectConstants.defaultDoctorStartTime).getTime());

        //Get end time rounded to next/previous 30 minutes
        long endTime = this.roundTo30Minutes((
                (doctor.getEndTime() != null)
                        ? doctor.getEndTime(): ProjectConstants.defaultDoctorEndTime).getTime());

        HashMap<String,List<String>> availability=new HashMap<>();
        for (DayOfWeek weekDay: DayOfWeek.values()){
           if(this.isDoctorAvailableForDay(doctor,weekDay)) {
               var upComingAppointments = this.appointmentRepository
                       .getAppointmentsByDoctorIdAndStatusAndWeekDay(
                               doctor.getId(),
                               AppointmentStatus.BOOKED,
                               weekDay);
               availability.put(
                       weekDay.toString().toLowerCase(),
                       this.getListOf30minutesTimeSlots(startTime, endTime, upComingAppointments));
           }
        }
        return  availability;
    }
    ...
    private List<String> getListOf30minutesTimeSlots(long startTime,long endTime,List<Appointment> bookedAppointments){
        ...
        while (startTime <= endTime) {
            ...
            long conflictingAppointmentCount = bookedAppointments
                    .stream()
                    .filter(appointment -> {
                        var slotStartTimeWithCorrectDate = Timestamp.valueOf(slotStartTime.toLocalDateTime().with(appointment.getStartTime().toLocalDateTime().toLocalDate()));
                        boolean isConflicting= this.isAppointmentConflicting(appointment,slotStartTimeWithCorrectDate);
            ...
            // Move to the next time slot by adding 30 minutes in start time
    ...
    /**
     * Checks if given appointment is conflicting with doctors start and end time or not
     * @param appointment Appointment details
     * @param slotStartTime Start time of appointment request slot
     * @return Boolean status
     */
    private boolean isAppointmentConflicting(Appointment appointment,Timestamp slotStartTime){
        long startTime=appointment.getStartTime().getTime();
        long endTime=appointment.getEndTime().getTime();
        long slotStartTimeInEpoch=slotStartTime.getTime();
        return startTime <= slotStartTimeInEpoch && slotStartTimeInEpoch <= endTime;
    }
