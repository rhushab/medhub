...
public class CommonService implements ICommonService {
    ...
    public DoctorDetail convertDoctorToDoctorDetail(Doctor doctor) {
        ...
        doctorDetail.setSpecializationsOfDoctor(
                doctor.getSpecializationsOfDoctor()
                        .stream()
                        .map(SpecializationOfDoctor::getSpecialization)
                        .toList());
        ...
        if(StringUtils.isBlank(doctor.getProfilePictureFileName())){
            ...
            doctorDetail.setProfilePictureBase64String(
                    UtilityMethods.getBase64StringFromImage(
                            this.customConfigurations.getProfilePictureFolderPath(),
                            doctor.getProfilePictureFileName()));
        }
        return doctorDetail;
    }
    ...
    public AppointmentDetail getAppointmentDetailModelFromAppointment(Appointment appointment){
        ...
        appointmentDetail.setTimeSlot(this.getTimeSlotString(appointment));
    ...
    /**
     * Converts start time and end time to time slot string by combining them
     * @param appointment Appointment model
     * @return Time slot string
     */
    private String getTimeSlotString(Appointment appointment){
        SimpleDateFormat timeFormat = new SimpleDateFormat("HH:mm");
        return timeFormat.format(appointment.getStartTime())
                + " to " + timeFormat.format(appointment.getEndTime());
    }
